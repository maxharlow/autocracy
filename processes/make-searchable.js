...
async function initialise(origin, destination, forceOCR, verbose, alert) {
    ...
    const sequence = [
        ...
        },
        {
            ...
            setup: () => autocracy.operations.convertJpegPagesToPDFPages(cacheJpegPages, cachePDFPages, 'shell', 'eng', 300, verbose, alert)
        },
        {
            name: 'Combining PDF pages',
            setup: () => autocracy.operations.combinePDFPages(cachePDFPages, destination, 'shell', verbose, alert)
        }
    ]
